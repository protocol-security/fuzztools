...
contract DelegateForwarder {
    // This contract will be used for nested delegatecall tests
    fallback() external payable {
        // Forward the delegatecall to the address encoded in calldata
        assembly {
            // First 32 bytes of calldata should be the target address
            let target := calldataload(0)
            
            // Copy remaining calldata (if any) after the address
            let dataSize := sub(calldatasize(), 32)
            calldatacopy(0, 32, dataSize)
            
            // Delegatecall to the target with the remaining data
            let success := delegatecall(gas(), target, 0, dataSize, 0, 0)
